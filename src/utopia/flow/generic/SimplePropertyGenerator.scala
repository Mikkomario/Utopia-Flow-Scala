 ...
 */
class SimplePropertyGenerator[T <: Property](val createProperty: (String, Value) => T, 
        val defaultValue: Value = Value.empty()) extends PropertyGenerator[T] with Equatable
{
    // IMPLEMENTED    ------------
    
    override def properties = Vector(createProperty, defaultValue)
